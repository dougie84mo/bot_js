    ...
    class Amazon extends BaseMonitor {
        ...
        async run() {
            ...
            await this.browser_init(
                async function (page) {
                    console.log('Working callable');
                }
            );
        }

        async check_login(page, profile) {
        ...
        async check_password_page(page, profile, bypass=false) {
            let password_field;
            if (bypass) {
        ...
        async check_if_available(page) {
            let cart_button = await page.$("#add-to-cart-button");
            return cart_button;
        }
