...
const {env, deb, conf} = require('./config');
const fs = require('fs');
const webhook = require("webhook-discord");
const hat = require('hat');
const Mongo = require('mongodb');
const readline = require("readline-sync");
const MongoCl = new Mongo.MongoClient(process.env.ATLAS_URI, { useNewUrlParser: true, useUnifiedTopology: true });
// const data_dir = '.\\..\\data\\';
// const profiles_directory = `${data_dir}profiles\\`;
// const address_path = `${data_dir}addresses.json`;
const c = (key, value) => {console.log(`[${key}]: ${value}`);};
...
class DB {
    ...
    static create_client(options = { useNewUrlParser: true, useUnifiedTopology: true }) {
        return new Mongo.MongoClient(process.env.ATLAS_URI, options)
    }

    static init_std_connect(callback) {
        (async () => {
            await DB.std_connect(callback);
        })();
    }

    static async std_connect(callback) {
        await MongoCl.connect();
        let db = MongoCl.db(process.env.DB_NAME);
        await callback(db);
    }
    ...
    static async getApiTokens() {
        await MongoCl.connect();
        let db = MongoCl.db(process.env.DB_NAME);
        return await db.collection('apiTokens').find();
    }

    static generateToken({amount, email = '', name = 'base_token', token_access = 0}) {
        let gen_key = {
            "apiKey": hat(256),
            "name": name,
            "keyType": token_access,
            "email": email,
...
class Pro {
    ...
    /*Command Line forms*/
    static choosable_array(choices, question='What choice do you pick?', hasExitCode=false) {
        let temp_arr = {};
        if (hasExitCode) {
            temp_arr[-1] = hasExitCode;
        }
        let choice_keys = Object.keys(choices);
        // console.log(choice_keys, choice_keys.length-1);
        for (let i = 0; i <= choice_keys.length-1; i++) {
            deb.high(i)
            temp_arr[i] = choice_keys[i];
        }
        let answer = null;
        while (answer === null) {
            Object.keys(temp_arr).map((key, value)=> {
               c(key, temp_arr[key]);
            });
            let response = readline.questionInt(question);
            if (response <= Object.keys(temp_arr).length - 1) {
    ...
    static choosable_list(choices, question='What choice do you pick?') {
        let temp_arr = [];
        for (let [key, value] in Object.entries(choices)) {
    ...
    static choose_int(range, question='What choice do you pick?\n') {
        let answer = null;
        let range_min = range.length === 2 ? range[1] : 0;
        let range_max = range[0] > 100 ? 100 : range[0];
        while (answer === null) {
            let num = readline.questionInt(question);
            if (range_max >= num && range_min <= num) {
                answer = num;
            } else {
                console.log('Choice is not available');
            }
    ...
    static choose_str(range, question='What name?\n') {
        let answer = null;
        while (answer === null) {
            let num = readline.question(question);
            if (num.length <= range[0]) {
                answer = num;
            } else {
                console.log('Choice is not available');
            }
