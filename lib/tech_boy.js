...
// const uri = "mongodb+srv://dougie:pictureparchbacillus@bottestcluster.pqzgo.mongodb.net/BotTestCluster?retryWrites=true&w=majority";
// const data_dir = '.\\..\\data\\';
...
class DB {
    ...
    static create_client(options = { useNewUrlParser: true, useUnifiedTopology: true }) {return new Mongo.MongoClient(process.env.ATLAS_URI, options)}

    static async init_mongo_cluster() {
        let database = await DB.std_async_connect();


    }

    static async std_async_connect(dbName="botJs") {
        await MongoCl.connect();
        return MongoCl.db(dbName)
        // Do something
    }

    static open_mango(callback) {
        MongoCl.connect().then(result => {

        }, err => {
            const collection = MongoCl.db("proxies").collection("devices");
            callback();
        });
        MongoCl.close();
        // TODO: connect and close callbacks?, promise returns
    }
}
